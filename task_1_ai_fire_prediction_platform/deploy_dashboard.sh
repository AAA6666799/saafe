...
ECR_REPOSITORY="saafe-fire-dashboard"
CLUSTER_NAME="saafe-fire-cluster"
TASK_FAMILY="saafe-fire-dashboard"
SERVICE_NAME="saafe-fire-dashboard-service"
IMAGE_TAG="latest"
...
ROLE_NAME="SaafeFireDashboardExecutionRole"
...
SG_ID=$(aws ec2 create-security-group --group-name saafe-fire-dashboard-sg --description "Security group for Saafe fire dashboard" --vpc-id $VPC_ID --region $REGION --query 'GroupId' --output text 2>/dev/null || aws ec2 describe-security-groups --filters "Name=group-name,Values=saafe-fire-dashboard-sg" --region $REGION --query 'SecurityGroups[0].GroupId' --output text)
