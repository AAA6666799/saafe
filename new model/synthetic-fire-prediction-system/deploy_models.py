...
This script deploys trained models using SageMaker hosting services for the FLIR+SCD41 fire detection system.
"""
...
def deploy_model(model_name, model_data_url, algorithm_type='sklearn'):
    ...
    deployment_name = f"flir-scd41-{algorithm_type}-{timestamp}"
...
if __name__ == "__main__":
    ...
    # Deploy models using existing trained model artifacts
    print("\nDeploying models using existing trained artifacts...")
    
    # Deploy XGBoost model
    xgb_model_artifacts = "s3://fire-detection-training-691595239825/flir_scd41_training/models/flir-scd41-xgboost-simple-20250828-154649/output/model.tar.gz"
    ...
    # Deploy ensemble debug model
    ensemble_debug_model_artifacts = "s3://fire-detection-training-691595239825/flir_scd41_training/models/flir-scd41-ensemble-debug-20250901-110038/output/model.tar.gz"
    ensemble_debug_endpoint, ensemble_debug_model, ensemble_debug_config = deploy_model(
        "ensemble-debug-model", 
        ensemble_debug_model_artifacts, 
        "sklearn"
    )
    
    # Deploy full ensemble model
    full_ensemble_model_artifacts = "s3://fire-detection-training-691595239825/flir_scd41_training/models/flir-scd41-full-ensemble-fixed2-20250901-103338/output/model.tar.gz"
    full_ensemble_endpoint, full_ensemble_model, full_ensemble_config = deploy_model(
        "full-ensemble-model", 
        full_ensemble_model_artifacts, 
        "sklearn"
    )
