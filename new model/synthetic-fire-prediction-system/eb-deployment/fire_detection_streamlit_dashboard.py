...
# Live data streaming
def live_data_streaming():
    """Live data streaming component."""
    # Title and header
    st.title("🔥 Fire Detection System - Live Data Stream")
    st.markdown("---")
    ...
    # Auto-refresh
    st.subheader("Live Data Stream Manager")
    refresh_placeholder = st.empty()
    ...
    # Last updated
    from datetime import datetime
    st.caption(f"Last updated: {datetime.fromisoformat(stream_status['timestamp']).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    ...
    with col4:
        if st.button("🔄 Clear Data", key="clear_data"):
            if 'sensor_data' in st.session_state:
                del st.session_state.sensor_data
            st.rerun()
    ...
    if not sensor_data:
        st.error("Failed to retrieve sensor data. Please check data source.")
        return
    ...
    # Plot live data
    st.subheader("Live Data Plot")
    fig = plot_sensor_data(sensor_data)
    st.plotly_chart(fig, use_container_width=True)
    
    # Footer
    st.markdown("---")
    from datetime import datetime
    st.caption(f"Dashboard refreshed at: {datetime.fromisoformat(stream_status['timestamp']).strftime('%Y-%m-%d %H:%M:%S UTC')} | "
               f"AWS Account: 691595239825 | Region: us-east-1")
...
def get_sensor_data():
    ...
    try:
            ...
            for page in gas_pages:
                if 'Contents' in page:
                    for obj in page['Contents']:
                        ...
                        if file_time > one_minute_ago.replace(tzinfo=file_time.tzinfo):
                            recent_files.append({
                                'key': obj['Key'],
                                'size': obj['Size'],
                                'modified': obj['LastModified'].strftime('%Y-%m-%d %H:%M:%S UTC')
        ...
        data['recent_thermal_files'] = thermal_files
        data['recent_gas_files'] = gas_files
        data['recent_files'] = recent_files[:10]  # Show only top 10 recent files
        data['has_recent_data'] = len(recent_files) > 0

        # Process each file and extract readings
        for file_info in recent_files:
            try:
                ...
                # Parse the content (assuming it's a JSON array)
                readings = json.loads(content)
                
                # Add the readings to the data dictionary
                data['readings'].extend(readings)
            except Exception as e:
                pass  # Continue even if a file cannot be processed
    except Exception as e:
        data = {
            'error': str(e),
            'total_files': 0,
            'recent_thermal_files': 0,
            'recent_gas_files': 0,
            'recent_files': [],
...
def plot_sensor_data(sensor_data):
    ...
    gas_readings = [r.get('gas_value', None) for r in sensor_data['readings']]
    thermal_readings = [r.get('thermal_value', None) for r in sensor_data['readings']]
    
    # Create subplots
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                      subplot_titles=("Gas Sensor Readings", "Thermal Sensor Readings"))
    
    # Add gas readings to the first subplot
    fig.add_trace(
        go.Scatter(x=timestamps, y=gas_readings, mode='lines', name='Gas Sensor'),
        row=1, col=1
    )
    
    # Add thermal readings to the second subplot
    fig.add_trace(
        go.Scatter(x=timestamps, y=thermal_readings, mode='lines', name='Thermal Sensor'),
        row=2, col=1
    )
    
    # Update layout
    fig.update_layout(title='Sensor Data', showlegend=True)
    fig.update_xaxes(title='Timestamp')
    fig.update_yaxes(title='Reading')
...
def get_stream_status():
    """Get real-time status of the data streaming."""
    # Import datetime and timedelta locally for Streamlit context
    from datetime import datetime, timedelta
    ...
    if not sensor_data or not sensor_data.get('readings'):
        return status
    
    # Check if there is data being streamed
    ...
    if readings:
        ...
        if duration > 0:
            status['readings_per_second'] = len(readings) / duration
