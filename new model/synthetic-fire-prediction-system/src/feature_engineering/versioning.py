"""
Feature Versioning System for the synthetic fire prediction system.

This module provides functionality for tracking versions of extracted features,
maintaining feature lineage information, and managing feature metadata.
"""
...
class FeatureVersioningSystem:
    ...
    def prune_old_versions(self, max_versions_per_type: Optional[int] = None) -> int:
        ...
        """
        if max_versions_per_type is None:
            max_versions_per_type = self.config.get('max_versions', 10)
        ...
        for feature_type, versions in feature_types.items():
            ...
            if len(sorted_versions) > max_versions_per_type:
                ...
                for version_id, _ in versions_to_prune:
                    ...
                    with self.graph_lock:
                        if version_id in self.version_graph:
                            feature_id = self.version_graph.nodes[version_id].get('feature_id')
                    ...
                    with self.graph_lock:
                        if version_id in self.version_graph:
                            self.version_graph.remove_node(version_id)
                    
                    # Delete feature data if it's not referenced by other versions
                    if feature_id:
                        referenced = False
                        with self.graph_lock:
                            for _, data in self.version_graph.nodes(data=True):
                                if data.get('feature_id') == feature_id:
                                    referenced = True
                                    break
                        ...
                        if not referenced:
                            # Delete feature data
                            self.storage.delete_features(feature_id)
