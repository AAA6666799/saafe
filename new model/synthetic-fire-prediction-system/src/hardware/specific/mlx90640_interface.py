"""
Grove MLX90640 Interface Implementation.

This module provides a concrete implementation of the ThermalSensorInterface
specifically for the Grove MLX90640 thermal camera.
"""
...
class MLX90640Interface(ThermalSensorInterface):
    """
    Grove MLX90640 thermal camera interface implementation.
    
    This class provides a concrete implementation for interfacing with
    the Grove MLX90640 thermal camera, supporting its specific features
    and data formats.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize the Grove MLX90640 interface.
        
        Args:
            config: Dictionary containing MLX90640 configuration parameters
        """
        # Set attributes before calling super().__init__
        self.device_address = config.get('device_address', 0x33)  # Default I2C address
        self.resolution = config.get('resolution', (32, 24))  # 32x24 pixels for MLX90640
        self.temperature_range = config.get('temperature_range', (-40.0, 300.0))  # Celsius
        self.frame_rate = config.get('frame_rate', 8.0)  # Hz (max 8Hz for MLX90640)
        ...
        try:
            ...
            self.logger.warning("Adafruit MLX90640 library not available. Using mock implementation.")
            self.board = None
            self.busio = None
        ...
        self.logger.info(f"Initialized Grove MLX90640 interface for device address: 0x{self.device_address:X}")
    
    def validate_config(self) -> None:
        """
        Validate the Grove MLX90640 configuration parameters.
        ...
        # Convert list to tuple if needed for consistent comparison
        if isinstance(self.resolution, list):
            self.resolution = tuple(self.resolution)
        
        if self.resolution != (32, 24):
            raise ValueError("MLX90640 resolution must be (32, 24)")
    ...
    def connect(self) -> bool:
        """
        Connect to the Grove MLX90640 thermal camera.
        ...
        try:
            ...
            if not self.adafruit_mlx90640:
                self.connected = True
                self.logger.info("Using mock connection for MLX90640")
                return True
            ...
            self.connected = True
            self.logger.info(f"Connected to Grove MLX90640 at address 0x{self.device_address:X}")
            return True
        except Exception as e:
            self.logger.error(f"Failed to connect to Grove MLX90640: {str(e)}")
            return False
    
    def disconnect(self) -> bool:
        """
        Disconnect from the Grove MLX90640 thermal camera.
        ...
        try:
            ...
            self.connected = False
            self.logger.info("Disconnected from Grove MLX90640")
            return True
        except Exception as e:
            self.logger.error(f"Error disconnecting from Grove MLX90640: {str(e)}")
            return False
    
    def read(self) -> Dict[str, Any]:
        """
        Read data from the Grove MLX90640 thermal camera.
        
        Returns:
            Dictionary containing MLX90640 thermal data in the format expected by
        ...
        if not self.connected:
            raise RuntimeError("Grove MLX90640 not connected")
        ...
        try:
            ...
            features['device_type'] = 'grove_mlx90640'
            features['thermal_frame'] = thermal_image
            
            self.last_frame_time = datetime.now()
            self.logger.debug("Successfully read data from Grove MLX90640")
            
            return features
            
        except Exception as e:
            self.logger.error(f"Error reading from Grove MLX90640: {str(e)}")
            raise
    
    def get_status(self) -> Dict[str, Any]:
        """
        Get the status of the Grove MLX90640 thermal camera.
        
        Returns:
            Dictionary containing MLX90640 status information
        """
        return {
            ...
            'sensor_type': 'grove_mlx90640'
        }
    
    def calibrate(self) -> bool:
        """
        Calibrate the Grove MLX90640 thermal camera.
        ...
        try:
            ...
            self.logger.info("Calibrated Grove MLX90640")
            return True
        except Exception as e:
            self.logger.error(f"Error calibrating Grove MLX90640: {str(e)}")
            return False
    
    def get_thermal_image(self) -> np.ndarray:
        """
        Get a thermal image from the Grove MLX90640 sensor.
        
        Returns:
            2D numpy array representing the thermal image (32x24)
        """
        if not self.connected:
            raise RuntimeError("Grove MLX90640 not connected")
        ...
        try:
            ...
            # Convert to numpy array and reshape to 32x24
            thermal_image = np.array(frame).reshape(self.resolution)
            return thermal_image
            
        except Exception as e:
            self.logger.error(f"Error getting thermal image from Grove MLX90640: {str(e)}")
    ...
    def get_resolution(self) -> Tuple[int, int]:
        """
        Get the resolution of the Grove MLX90640 sensor.
        
        Returns:
            Tuple of (width, height) in pixels - (32, 24) for MLX90640
        """
        return self.resolution
    
    def get_temperature_range(self) -> Tuple[float, float]:
        """
        Get the temperature range of the Grove MLX90640 sensor.
...
# Convenience function for creating MLX90640 interface
def create_mlx90640_interface(config: Dict[str, Any]) -> MLX90640Interface:
    """
    Create a MLX90640 interface with default configuration.
    
    Args:
        config: Configuration dictionary for MLX90640
        
    Returns:
        Configured MLX90640Interface instance
    """
    default_config = {
        'device_address': 0x33,
        'resolution': (32, 24),
    ...
    return MLX90640Interface(default_config)