...
def train_random_forest_model(X_train, y_train, X_val, y_val):
    ...
    return rf_model, rf_train_metrics, rf_val_metrics
...
def train_gradient_boosting_model(X_train, y_train, X_val, y_val):
    ...
    gb_val_pred_proba = gb_model.predict_proba(X_val)[:, 1]
    ...
    return gb_model, gb_train_metrics, gb_val_metrics
...
def train_logistic_regression_model(X_train, y_train, X_val, y_val):
    ...
    return lr_model, lr_train_metrics, lr_val_metrics
...
def evaluate_ensemble(X_test, y_test, models, weights, feature_names):
...
def save_models_and_results(df, models, ensemble_weights, 
...
def main():
    ...
    try:
        ...
        # Train Random Forest model
        rf_model, rf_train_metrics, rf_val_metrics = train_random_forest_model(X_train, y_train, X_val, y_val)
        models['random_forest'] = rf_model
        model_metrics['random_forest'] = rf_val_metrics
        
        # Train Gradient Boosting model
        gb_model, gb_train_metrics, gb_val_metrics = train_gradient_boosting_model(X_train, y_train, X_val, y_val)
        models['gradient_boosting'] = gb_model
        model_metrics['gradient_boosting'] = gb_val_metrics
        
        # Train Logistic Regression model
        lr_model, lr_train_metrics, lr_val_metrics = train_logistic_regression_model(X_train, y_train, X_val, y_val)
        ...
        ensemble_preds, ensemble_probs, ensemble_metrics = evaluate_ensemble(
            X_test, y_test, models, ensemble_weights, feature_names
        )
